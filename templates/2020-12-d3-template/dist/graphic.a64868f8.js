...
var fonts = '"Lab Grotesque","LabGrotesque","LabGrotIns",sans-serif';
var fontsSerif = 'TiemposTextWeb, Georgia, Times, serif';
var black = '#000';
var offblack = '#222';
var gray4 = '#BCBCBC';
var gray3 = '#D3D3D3';
...
var orange2 = '#F37D00';
var orange1 = '#FFB0B0';
var pymChild = null; // for embedding
...
var svg = d3.select('#embed-graphic').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
function mouseOver(d, i) {
  ...
  d3.select("#".concat(d.country.replace(/[^0-9a-z]/gi, ''))).style('fill', blue2).transition().attr('r', 10); // add custom tooltip html

  tooltip.attr('data-html', 'true').style('visibility', 'visible').html("<div class='row'><b>".concat(d.country, "</b></div>"));
}
...
function mouseMove(d, widthEl) {
  ...
  var offset = d3.select('#embed-graphic').node().getBoundingClientRect().x;
  var parsedX = x + toolTipMargin;
  if (parsedX > widthEl / 2 + toolTipMargin * 2 + offset) parsedX = parsedX - toolTipWidth - toolTipMargin;
  tooltip.style('left', "".concat(parsedX, "px")).style('top', "".concat(y + toolTipMargin, "px"));
}

function mouseOut(d) {
  d3.selectAll('circle').transition().attr('r', 3).style('fill', blue2);
  tooltip.style('visibility', 'hidden');
}

function getExtent(data, column, number) {
  if (number === true) return d3.extent(data, function (d) {
    return +d[column];
  });else return d3.extent(data, function (d) {
    return d[column];
  });
}
...
Promise.all([d3.csv(require('/data/countries.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  xScale.domain(getExtent(data, 'gdp_per_capita', true));
  yScale.domain(getExtent(data, 'life_expectancy', true));
  var yOptions = d3.axisLeft(yScale).tickPadding(15);
  var yAxis = svg.append('g').attr('class', 'axis y-axis');
  var xOptions = d3.axisBottom(xScale);
  var xAxis = svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')');
  var circles = svg.selectAll('circle').data(data).join('circle').attr('r', 3).attr('id', function (d) {
    return d.country.replace(/[^0-9a-z]/gi, '');
  ...
  function render() {
    ...
    actualSvg.attr('width', svgWidth); // .attr('height',height) // include if you need to update the height
    // update the ranges

    xScale.range([0, width]);
    yScale.range([height, 0]); // update what is drawn
    ...
    circles.attr('cx', function (d) {
      ...
      return mouseMove(d, svgWidth);
    }).on('mouseout', function (d) {
      return mouseOut(d);
    });
    yAxis.call(yOptions.tickSizeInner(-width).tickPadding(15)).call(function (g) {
      return g.select('.domain').remove();
    });
    xAxis.call(xOptions.tickSizeInner(-height).ticks(width / 100).tickPadding(15)).call(function (g) {
      return g.select('.domain').remove();
  ...
  render();
  window.addEventListener('resize', render); // for the embed, don't change!
...
},{"d3":"../node_modules/d3/index.js","/data/countries.csv":"data/countries.csv"}]},{},["scripts/graphic.js"], null)
